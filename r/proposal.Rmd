...
```{r}
summary(dat$age_at_index)
```

Diagnoses occurred between ages 26-90. 11 patients were missing dates of diagnosis but had time to death data. We removed these subjects from the time-to-diagnosis analysis as the data we missing, not censored. However, these subjects had time-to-death data available so they were included in the time-to-death analysis.
```{r}
summary(dat$age_at_diagnosis_years)
```
...
We will analyze both the time to diagnosis and time to death. The time to diagnosis is uncensored as all subjects were diagnosed within the study but the time to death is censored as only half the subjects died.


```{r}
fit <- survfit(Surv(age_at_index, age_at_diagnosis_years, obs) ~ 1, 
                    data = dat %>% filter(!is.na(age_at_diagnosis))
               %>% mutate(obs = 1))
